...
Since this is a pretty long and complicated piece of code, 
    I'll leave some notes here and there to remind myself of things when 
    I eventually need to come back to read, refactor, or just study it.

Note: These notes will also serve to help my friends or 
    anyone else reading this who wants to check it out, 
    in order to better understand how this code works.
...
namespace lve {
    ...
    LveDevice::LveDevice(LveWindow& window) : window{ window } {
        // Create vulkan instance, initializing the vulkan library and 
        // its the connection between our application and vulkan
        createInstance();

        // Setup validation layers;
        // By default the vulkan api does extremely little error checking, meaning even small errors wont be 
        //  caught and can mess things up (even crashing) so when debugging we should enable validations layers to check for errors 
        //  and then disabling for release builds when performance is critical
        setupDebugMessenger();

        // Create our surface that is the connection between the window and vulkan ability to display results
        createSurface();

        // We pick the physical device our application will be using
        // The physical device is the graphics device in your system capable of working with the vulkan API
        pickPhysicalDevice();

        // This defines what features we want to use from the physical device
        createLogicalDevice();

        // Setup the command pool for later use
        createCommandPool();
    }
